...
- konsep gripper/claw dipakai untuk mekanisme attach/release payload. Mesh Beaumont sekarang tersedia sebagai opsi `rov_variant:=beaumont` untuk pembanding visual, tetapi default tetap `github_blue` karena lebih proporsional dengan arena KKI.
...
- default baru memakai `rov_variant:=github_blue`, yaitu mesh BlueROV2 dan T200 propeller dari GitHub `evan-palmer/blue` ditambah gripper bawah-depan custom compact yang dibuat sebagai geometri SDF agar menyatu dengan body ROV. Asset Ricketts tetap disimpan sebagai referensi lokal, tetapi visual aktif claw sudah dibuat ulang. Varian `github_blue_joint` sekarang memakai jalur compact stabil yang sama agar command latihan tidak bug, sedangkan `github_blue_joint_experimental` menyimpan rahang link + revolute joint untuk uji contact solver Gazebo.
...
- pergerakan default dibuat kinematic dari PWM thruster hasil allocator karena kombinasi rigid-body kecil + DART/ODE pada device ini membuat model dinamik bawah air crash saat smoke test,
